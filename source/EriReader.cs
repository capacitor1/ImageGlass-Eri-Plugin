...
namespace EriCodec
{
    internal class EriReader
    {
        EriMetaData     m_info;
        byte[]          m_output;
        Misc.ERISADecodeContext m_context;
        int             m_dst;
...
//        float[]         m_ptrBuffer2;
        ...
        public EriPixelFormat    Format { get; private set; }
        public int            Stride { get { return Math.Abs (m_dwBytesPerLine); } }
        public RgbColor[] Palette { get; private set; } = Array.Empty<RgbColor>();

        public EriReader (Stream stream, EriMetaData info, RgbColor[] palette, byte[] key_frame = null)
        {
            m_info = info;
...
//                (context as ArithmeticContext).InitArithmeticContext (8);
            ...
            {
                if (context.DecodeBytes (m_ptrLineBuf, (uint)m_info.Width) < m_info.Width)
                    throw new InvalidFormatException();
        ...
        private void DecodeLossyImage (HuffmanDecodeContext context)
        {
            ...
            uint nWidthDivBlocks  = (uint)(m_info.Width + (nBlockSize - 1)) / nBlockSize;
            uint nHeightDivBlocks = (uint)(m_info.Height + (nBlockSize - 1)) / nBlockSize;
        ...
        PtrProcedure GetLLRestoreFunc (EriType fdwFormatType, int dwBitsPerPixel)
        {
            ...
            case 32:
                ...
                {
                    Format = EriPixelFormat.Bgra32;
                    if (null == m_src_frame)
                        return RestoreRGBA32;
                    else
                        return RestoreDeltaRGBA32;
                }
                Format = EriPixelFormat.Bgr32;
                if (null == m_src_frame)
                    return RestoreRGB24;
                else
                    return RestoreDeltaRGB24;
            case 24:
                Format = EriPixelFormat.Bgr24;
                if (null == m_src_frame)
                    return RestoreRGB24;
                else
                    return RestoreDeltaRGB24;
            case 16:
                Format = EriPixelFormat.Bgr555;
                return RestoreRGB16;
            case 8:
                if (null == Palette)
                    Format = EriPixelFormat.Gray8;
                else
                    Format = EriPixelFormat.Indexed8;
                return RestoreGray8;
            }
            return null;
        }
    ...
    internal class RLEDecodeContext : Misc.ERISADecodeContext
    {
